...
{{% img src="post/synthesis-for-architects/oracle-table.png" alt="oracle-guided synthesis test cases" width="65%" %}}
...
In the example, that next trip around the loop will be the last: the only way to use two adds and a square root to satisfy the (now three) test cases is the program `sqrt(x+y)`. The question the verifier asks will therefore be unsatisfiable -- it won't be able to find a program *P'* -- and so the loop will exit.
...
The authors use two domains to illustrate why their technique is useful. The first is the traditional suite of bitvector benchmarks from [Hacker's Delight][hackers]. Many synthesis papers use these benchmarks because they are examples of small, unintuitive programs. Instead of trying to come up with the most efficient bit-twiddling hack, programmers can write a simple, inefficient implementation of a bitvector manipulation, and the synthesiser uses this implementation to produce an optimal program. The second domain is program deobfuscation -- taking an obfuscated program as the oracle, and synthesising a new, simpler program that matches its behaviour.
