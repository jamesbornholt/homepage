...
In an automated reasoning tool, however,
the inputs to this function may not be concrete---for example,
`lst` may contain unknown (symbolic) numbers,
and the prefix size `k` may itself be unknown.
This uncertainty is because an automated reasoning tool
is *searching* for inputs that make the program fail (verification),
or for a program that makes it succeed on every input (synthesis).
